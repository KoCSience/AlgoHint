...
Gemini障害の診断ログには、provider、model、安全化したreason code、HTTPステータス、
再試行可能性、例外クラスだけを記録します。APIキー、リクエスト、提出コード、
Googleの生レスポンスと例外メッセージは記録しません。`algohint doctor`もモデル情報の
取得だけを行い、問題文、コード、質問、会話履歴を送信しません。
