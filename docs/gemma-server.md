...
ssh -T "$ALGOHINT_SSH_TARGET" 'true'
```
...
AlgoHintを動かすhostへ、同じAPI keyを権限制限したfileとして転送します。

```bash
exit
install -d -m 700 "$HOME/.config/algohint"
scp "$ALGOHINT_SSH_TARGET:.config/algohint-gemma-server/credentials" \
  "$HOME/.config/algohint/gemma-remote-credentials"
chmod 600 "$HOME/.config/algohint/gemma-remote-credentials"
```

Gemmaだけを使う起動では、次のようにこのfileをAlgoHint launcherへ指定できます。
他providerのkeyも使う場合は、安全なeditorで利用者管理のcredentials fileへ必要な
変数代入だけを統合します。
...
stackを起動し、同じcredentials環境でmodel discoveryだけを確認します。
...
| `401` | local/remote API key不一致。keyを表示せず再転送・rotation |
